...
double PedoneCheck::calcGrad(double label) {
    ...
    //std::cout<<"error "<<std::pow(_risRete.output - label, 2.0) / 2.0<<std::endl;
    return std::pow(_risRete.output - label, 2.0) / 2.0;
}

void PedoneCheck::updateweights(double l) {
    // output layer
    //std::cout<<"output  = "<< _pesi.biasOutput<<std::endl;
    _pesi.biasOutput -= l * _biasGrad.BiasGradOutput;
    //std::cout<<"upgrade Output = "<< l * _biasGrad.BiasGradOutput<<std::endl;
    //std::cout<<"output  = "<< _pesi.biasOutput<<std::endl;
    ...
    for( unsigned int i = 0; i <SizeLayer23; ++i) {
        ...
        /*if(std::abs(_pesi.pesiOutput[i] - _NNUEmodel->getLayer(3).weight()[i]) > 1e-7) {
            ...
            std::cout<<(_pesi.pesiOutput[i] - _NNUEmodel->getLayer(3).weight()[i])*1e7<<std::endl;
            exit(-1);
        }*/
    }
    ...
    /*for (unsigned int in = 0; in < MaxInput; ++in) {
        for (unsigned int out = 0; out < SizeLayer1/2; ++out) {
            if(std::abs(_pesi.pesiLayer1[in][out] - _NNUEmodel->getLayer(0).weight()[in * SizeLayer1 / 2 + out])>1e-7) {
